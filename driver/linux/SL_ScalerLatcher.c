...
volatile struct Read_data *SL_d[2];
volatile struct csr_struct *SL_p;
...
STATUS SL_Init (UINT32 addr)
{
  ...
  SL_p = (struct car_struct *)( laddr );
  for ( ii=0; ii<2; ii++ ){
    SL_d[ii] = (struct Read_data *)(   laddrd + ii*addr_inc );
  }
