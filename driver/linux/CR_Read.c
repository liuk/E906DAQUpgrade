...
/*******************************************************************************
*
* CR_Init - Initialize SIS 3600 Library.
*
...
STATUS CR_Init(UINT32 addr, UINT32 addr_inc, int nmod)
{
  ...
  if(addr == 0) {
    ...
    ress = vmeBusToLocalAdrs(0x09, (char*)addrs, (char**)&laddrs);
    if(ress != 0) {
    ...
    resd = vmeBusToLocalAdrs(0x09, (char*)addrd, (char**)&laddrd);
    if(resd != 0) {
    ...
    resdp = vmeBusToLocalAdrs(0x09, (char*)addrdp, (char**)&laddrdp);
    if(resdp != 0) {
      printf("CR_Init: ERROR in sysBusToLocalAdrs(0x09,0x%x,&laddrd) \n", addrdp);
      printf("CR_Init: ERROR resd=%d\n", resdp);
      return(ERROR);
...
{
  int tmp = vmeRead32(&CR_p[id]->reg[1]);
  vmeWrite32(&CR_p[id]->reg[1], tmp & 0xfffff7ff);
...
void CR_TrigEnable(int id)
{
  int tmp = vmeRead32(&CR_p[id]->reg[1]);
  tmp = tmp & 0xfffff7ff;
  vmeWrite32(&CR_p[id]->reg[1], tmp + 0x800);
...
{
  vmeWrite32(&CR_p[id]->reg[1], csr & 0xfffff7ff);
  return;
}
...
{
  csr = csr & 0xfffff7ff;
  vmeWrite32(&CR_p[id]->reg[1], csr + 0x800);
  return;
}
...
{
  if(scalarID < 8) {
    int org_regcc = vmeRead32(&CR_p[id]->reg[3]);
    vmeWrite32(&CR_p[id]->reg[3], (org_regcc & 0x00ffffff) + (scalarID << 24));
  } else {
...
{
  int org_regcc = vmeRead32(&CR_p[id]->reg[4]);
  if(inhibit <= 63) {
    vmeWrite32(&CR_p[id]->reg[4], (org_regcc & 0xffc0ffff) + (inhibit << 16));
  } else {
    printf("ERROR!! inhibit =%d. it should less than 63!! \n", inhibit);
  }

  org_regcc = vmeRead32(&CR_p[id]->reg[4]);
  vmeWrite32(&CR_p[id]->reg[4], org_regcc | 0x00800000);

  if(update == 1) {
    vmeWrite32(&CR_p[id]->reg[4], org_regcc|0x00600000);
  } else if(update == 0) {
    vmeWrite32(&CR_p[id]->reg[4], org_regcc & (~0x00600000));
  } else {
...
{
  vmeWrite32(&CR_p[id]->reg[regaddr], reg_value);
  return;
}

int CR_RD_Reg(int id, int regaddr)
{
  return(vmeRead32(&CR_p[id]->reg[regaddr]));
}

void CR_Clear(int id)
{
  vmeWrite32(&CR_p[id]->reg[3], 1);

  tim.tv_sec = 0;
  tim.tv_nsec = 1000;
  nanosleep(&tim, &tim2);

  vmeWrite32(&CR_p[id]->reg[3], 0);
  return;
}

void CR_Reset(int id)
{
  vmeWrite32(&CR_p[id]->reg[3], 0x1001);
...
void CR_HeaderInit(int id, int csr)
{
  ...
  for(ibuffer = 0; ibuffer < M_Num_buf[now_M_value]; ibuffer++) {
    ...
    vmeWrite32(&CR_d[id]->data[idata], 0);
  }
...
void CR_ScalarInit(int id, int scalarID)
{
  ...
  for(ich = 0; ich < 64; ich++) {
    vmeWrite32(&CR_s[id]->scalar[64*scalarID + ich], 0);
  }
  return;
}
...
void DP_Write(int id, int value, int dp_start, int dp_end)
{
  int idata;
  for(idata = dp_start; idata <= dp_end; ++idata) {
    vmeWrite32(&CR_dp[id]->dp[idata], value);
  }
}
...
void CR_ScalarDisplay(int id, int scalarID){
  ...
  for(ich = 0; ich < 64; ich++) {
    ...
    printf("%2d: %d, ", ich, vmeRead32(&CR_s[id]->scalar[ibuffer]));
    if((ich+1) % 8 == 0) printf("\n");
  }
...
void CR_DataDisplay(int id)
{
  ...
  for(idata = 0; idata < DATA_BUF_SIZE; idata++) {
    ...
    printf("%8x ", vmeRead32(&CR_d[id]->data[idata]));
  }
...
void CR_FifoRead(int id, int ii)
{
  ...
  for ( ii=0; ii<64; ii++ ){
    ...
    res[ii] = ( vmeRead32(&CR_p[id]->reg[ii]) ) & 0xffffffff;  /* --- CH 01~32 --- */
...
void checkTDC (int nloop,int csr){
  ...
  for (iloop=0; iloop<nloop; iloop++){
    ...
    for( idata=0; idata< DATA_BUF_SIZE; idata++){
      int tmpFine;
      int header= (vmeRead32(&CR_d[id]->data[idata])&0x80000000)>>31;
      if (vmeRead32(&CR_d[id]->data[idata])!=0 && header==0){
	tmpFine=vmeRead32(&CR_d[id]->data[idata])&0x7; ////fine tdc time
...
void checkTDC_Ch (int nloop,int csr){
  ...
  for (iloop=0; iloop<nloop; iloop++){
	...
	tmpCh= (vmeRead32(&CR_d[id]->data[idata])&0x0f000000)>>24;
	tmpFine=vmeRead32(&CR_d[id]->data[idata])&0xf; ////fine tdc time
	//	printf("%d:org=0x%x,  fineTDC[%d]=%d\n",idata,CR_d[id]->data[idata], tmpFine, fineTDC[tmpFine]);
...
void saveTDC (int nloop,int csr){
  ...
  for (iloop=0; iloop<nloop; iloop++){
    ...
    for( idata=0; idata< DATA_BUF_SIZE; idata++){
      int tmpFine,tmpCh;
      int tmpData=vmeRead32(&CR_d[id]->data[idata]);
