...
volatile struct Long_data *Long_d1[v1495_MAX_BOARDS];
...
void v1495_Set_Cable_Mask(int id, int cableID, int val)
{
    int index = 11;
    if(cableID == 1 || cableID == 2) {
        index = cableID + 10;
    } else {
        index = cableID + 12;
    }

    va495_p[id]->reg[index] = val;
}
...
STATUS WaveUnionTDCTest (void *addr,     /* address of v1495 pulser module*/
			 void *addrTDC,  /* address of TDC module*/
			 int NtotEvt)
{
  int N_rows=8;
  int  width=4;
  ...
  void *addrTDCdata;
  unsigned int laddr;
  unsigned int laddrTDC;
  unsigned int laddrTDCdata;
  unsigned long aaa;

  int BuffSt[4] ={0, 512, 256, 384};

  static int  lastWidth = 2;  /* last width - default to 2 */
  char *filename="WaveUnionTDC_out.txt";
  static void *lastAdrs = 0; /* last location displayed */

  addrTDCdata=addrTDC+0x1000;

  pFile = fopen (filename,"w+");
  res = sysBusToLocalAdrs (0x39, (char *) addr, (char **) &laddr);  /*0x39 is A32 non privileged data access */
  res = sysBusToLocalAdrs (0x09, (char *) addrTDC, (char **) &laddrTDC);  /*0x39 is A32 non privileged data access */
  res = sysBusToLocalAdrs (0x09, (char *) addrTDCdata, (char **) &laddrTDCdata);  /*0x39 is A32 non privileged data access */

  printf("addr=%x, laddr=%x\n",addr,laddr);
  printf("addrTDC=%x, laddrTDC=%x\n",addrTDC,laddrTDC);
  printf("addrTDCdata=%x, laddrTDCdata=%x\n",addrTDCdata,laddrTDCdata);
  Long_d[0]= (struct Long_data *) (laddrTDC);
  Long_d1[0]= (struct Long_data *) (laddrTDCdata);

  Long_d[0]->reg[1]=0xffff0162;
  for(ii=0;ii<4;ii++){
    aaa=Long_d1[0]->reg[ii];
    printf("%04x\n",aaa );
  }



  v1495PulserInit(addr, 0x100000, 1);
  v1495_p[0]->reg[0]=0x1100;
  v1495_p[0]->reg[7]=0x00A0;

  v1495_PulserStatus(0);


  for(kk=0;kk<NtotEvt;kk++){
    v1495_ExternalPulser(0);
    for(ii=0;ii<256;ii++){
      aaa=Long_d1[0]->reg[ii+BuffSt[kk&3]];
      /*
      printf("%04x\n",aaa );
      fprintf("%04x\n",aaa );
      */
